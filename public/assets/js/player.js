...
async function loadMedia() {
  ...
  document.querySelector('#playerSection').style.display = 'flex'
  fetch(`/play`);
  // firstRun = false
}

async function playMedia() { //playing after song has been paused
...
async function addRecents(queue) {
  ...
  let uploadRecents = await fetch(`/recents`, {
    method: 'POST', headers: {
...
async function saveSketch() {
  ...
  if (confirm('Are you sure you want to save? This sketchad will be reset after drawing has been saved to your account.')) {
    ...
    var save = await fetch(`/saveSketch`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json;charset=utf-8'
      },
